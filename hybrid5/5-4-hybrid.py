...
class Paths:
    ...
    BEST_HYBRID_MODEL = './5-4-best_hybrid.pth' 
    PLOT_IMG = './5-4-resnet_hybrid_optimized.png'
...
class VoltageMapper(nn.Module):
    def __init__(self, num_channels, target_mean=3.5): 
        # [偏置设定] 3.5V 配合 2.5V 的阈值，保证 Vov ~ 1.0V (强反型区)
        super().__init__()
...
class DifferentialEKVConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0):
        ...
        # [核心修正 - 阈值下移] 
        # 均值从 4.0V -> 2.5V。物理上强制器件开启。
        # 这是打破"梯度死锁"的关键：只有器件导通，才有梯度去调整增益和权重。
...
def main():
    ...
    # [核心修正] 细分优化器参数组，并精细控制 Weight Decay
    ekv_params = [p for n,p in net_hybrid.named_parameters() if 'theta' in n]
    
    # Mapper 和 TIA 增益是物理校准参数，绝不能使用 Weight Decay，否则增益会被强行压到 0
    ...
    optimizer = optim.SGD([
        {'params': ekv_params, 'lr': cfg.LR_EKV, 'weight_decay': 5e-4}, # 权重需要 decay
        {'params': calibration_params, 'lr': 0.02, 'weight_decay': 0.0}, # [重点] 校准参数不需要 decay！
    ...
    plt.title("Hybrid ResNet (Strong Inversion + No Decay on Gain)")
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy (%)")
